...
func InitUsers() {
	...
	(?, ?, ?, ?)`,
		...
		"librarian", "librarian@example.com", librarianPassword, "librarian",
		"reader", "reader@example.com", readerPassword, "reader")
...
INSERT INTO borrow_records 
        (user_id, book_id, borrow_date, due_date, return_date) VALUES 
        (?, ?, ?, ?, ?),
        (?, ?, ?, ?, ?),
        (?, ?, ?, ?, ?),
        (?, ?, ?, ?, ?)`,
		1, 1, twoWeeksAgo, twoWeeksAgo.AddDate(0, 0, 14), now,
		2, 2, oneWeekAgo, inOneWeek, nil,
		3, 3, oneWeekAgo, inTwoWeeks, nil,
		3, 4, twoWeeksAgo, overdueDate, nil)
